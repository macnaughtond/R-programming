...
for( j in 1:length(filesList)) #
{
	...
	for(i in 1:nrow(data)){
		...
		DrainAccumulator = DrainAccumulator + data$Drainage[i] # Alternatively write in shorthand: DrainAccumulator += data$Drainage[i]
		RunoffAccumulator = RunoffAccumulator + data$Runoff[i]
		SoilLossAccumulator = SoilLossAccumulator + data$soil_loss[i]
		current_year <- year
